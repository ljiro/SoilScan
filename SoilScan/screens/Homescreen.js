...
const HomeScreen = ({ navigation, route }) => {
  ...
  const uploadImageToAPI = async (fileUri) => {
    try {
      ...
      // Save photo to app-private storage (now optimized with compression and thumbnails)
      const savedPhoto = await savePhoto(fileUri, null, {
        soilType: formattedResult.name,
        confidence: formattedResult.confidence / 100,
      });
      
      if (savedPhoto) {
        setSavedPhotoUri(savedPhoto.uri);
        ...
        await addScanLog({
          ...
          thumbnailUri: savedPhoto.thumbnailUri,
          location: location,
